...
const PhotoDelete = () => {
  ...
  const fetchPhotos = async () => {
    try {
      const response = await fetch('http://10.0.1.104:8000/api/photo'); // Adjust the URL to your API endpoint
      if (response.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
        const response = await fetch(`http://10.0.1.104:8000/api/photo/${parsedPhotoID}`, {
            method: 'DELETE',
        });
        
        console.log(`Current Delete URL: http://10.0.1.104:8000/api/photo/${parsedPhotoID}`);
